...
interface TeamDisplayCardProps {
  ...
  isDraggable?: boolean;
  isScreenshotting?: boolean; // Added prop
}
...
export const TeamDisplayCard: React.FC<TeamDisplayCardProps> = ({
  ...
  onPlayerSwap,
  isDraggable = false,
  isScreenshotting = false // Default to false
}) => {
    ...
    if (sourceInfoString) {
      try {
        ...
        if (sourceInfo.name === targetInfo.name && sourceInfo.teamId === targetInfo.teamId) {
          // Dropped on self, do nothing or minimal UI reset
        } else {
  ...
  const handleDragEnd = (event: React.DragEvent<HTMLLIElement>) => {
    ...
    // Clean up any other lingering styles if necessary, though dragleave should handle highlights
  ...
  if (isScreenshotting) {
    ...
    playerNameStyle.bottom = '6px'; // Adjusted to move text 6px UP
  }
      ...
      {players.length > 0 ? (
        <ul className="space-y-3 flex-grow">
          {players.map((player, index) => (
            <li
              key={`${player}-${index}-${teamId}`} // Ensure unique key across all lists
